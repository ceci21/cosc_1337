...
understanding the code that is provided. It contains one working sort algorithm and
one working search algorithm. Your job is to add two new sort algorithms and one new
search algorithm. When complete, the program implements and tests three different
sort algorithms and two different search algorithms. (26 points max if you add one
sort and one search. 30 points max if you add two sorts and one search.)

The choice of new algorithms is yours. The textbook provides algorithms and explanations
for linear search and binary search; and source code and explanations for bubble sort
and selection sort. These are commonly used algorithms for searching and sorting.

Note: The basic program is provided on the assignment link. Selection sort and linear
search are already coded and tested. You have to add two more sorts and one more
search algorithm. In the code you provide, use variable names (often very cryptic
in textbooks) in your own terms, so the algorithm is easier to understand.

When you implement these algorithms, use your own variable names and comments,
which reflect your own personal understanding of how these algorithms work.
You can use the textbook code at first, but modify the names, and add comments, to
show your understanding of what it is doing. Test your code to verify it works by
sorting a small array properly.

Next, measure the performance of your algorithms and find out which is best.
Performance measuring code is provided. To measure the performance, the program gets
the time before and after your algorithm runs and subtracts the stop time from the
start time to get the duration, in milliseconds, of your algorithm. Example:
...
For small data sizes, the time will be 0 milliseconds. To get a useful duration, it
works better if it runs for at least 1/4 second (250 milliseconds). Two ways to get
an algorithm to run longer: 1) increase the data size; 2) place a loop around the
algorithm, so it runs many times. For example, perform a search 1000 times in a loop,
and divide the duration by 1000.0 to get the milliseconds for one iteration.

Performance testing code is provided. It will try five different array sizes. Try to
keep the maximum execution time to at most 10 seconds. Arrays of size:
5000, 10000, 20000, 30000, 40000, up to 100000 may work. Include the performance
results of your experimentation here. Example (for each entry, there is some number,
in milliseconds):

    Algorithm   1000, 10000, 20000, 30000, 40000  (Array size)
    ----------  ----  -----  -----  -----  -----
    sort1          x     xx    xxx   xxxx  xxxxx
    sort2          y     yy    yyy   yyyy  yyyyy
    sort3          z     zz    zzz   zzzz  zzzzz
    search1        m     mm    mmm   mmmm  mmmmm
    search2        n     nn    nnn   nnnn  nnnnn
Observe: As the size of the array increases, how does the duration increase?

Note: Code for automating the performance testing of the algorithms is included. Read
the code to understand how to performance test your own search and sort algorithms.
Very short run-times (fast algorithms, small data) may register as 0 time, which is
under 1 millisecond (1/1000th of a second).
*/
...
void testAlgorithms(int array[], int arraySize) {
  ...
  while (testSizes[testCount] && testSizes[testCount] <= arraySize)
    cout << setw(9) << right << testSelectionSort(array, testSizes[testCount++]);
  cout << endl;

  testCount = 0;
  cout << setw (10) << left << "linearSrch";
  while (testSizes[testCount] && testSizes[testCount] <= arraySize)
    cout << setw(9) << right << testLinearSearch(array, testSizes[testCount++]);
  cout << endl;
...
int main () {
  ...
  system("pause");  // optional. comment out if desired
  return 0;
} // end of main
...
Selection sort on bigArray took: 3838.00 milliseconds.
Linear search  of bigArray took:    0.16 milliseconds.

Algorithm             1000      5000     10000     20000     30000     40000     50000
================   =======   =======   =======   =======   =======   =======   =======
selection sort        0.00     32.00    156.00    624.00   1372.00   2465.00   3838.00
linear search         0.00      0.01      0.03      0.06      0.09      0.12      0.16
